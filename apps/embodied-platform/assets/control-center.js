/* 星聚 · Liquid Glass — 控制中心 Control Center glue (ops console).
   Wires the Control Center to live glass settings so a console-only user can
   dial refraction down — the same window.GlassRefraction + localStorage state
   the labeler writes, so the setting governs BOTH apps. Refraction itself
   auto-installs via /vendor/glass/refract.js (it finds the .glass slabs); this
   module ONLY adds controls and removes no existing behaviour.
   Mirrors apps/embodied-labeler/assets/console/labeler-glass.js (sans the
   focus + now-playing tiles, which the ops console Control Center omits). */
...
function wire() {
  ...
  // backdrop brightness (--cc-bright var)

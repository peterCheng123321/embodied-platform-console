...
class TestPostSegments:
    ...
    def test_post_without_signature_is_rejected(self, client):
        # #2: with no signed principal headers the request never reaches the
        # writer — the auth dependency rejects it with 403.
        actor = "alice"
        ann = _annotator_id(actor)
        body = {
            "episode_id": "demo_episode",
        ...
        r = client.post("/api/embodied/segments", json=body)  # no auth headers
        assert r.status_code == 403

    def test_post_with_forged_signature_is_rejected(self, client, tmp_path):
        # #2: a forged principal signature is rejected with 403 and nothing is
        # written, even though the actor/role headers are present.
        actor = "alice"
        ann = _annotator_id(actor)
        body = {
            "episode_id": "demo_episode",
        ...
        r = client.post("/api/embodied/segments", json=body, headers={
            "X-Embodied-Actor": actor,
...
class TestGetSegments:
    ...
    def test_get_without_signature_is_rejected(self, client):
        # #2 mirror: GET also requires a signed principal; no headers => 403.
        actor = "alice"
        ann = _annotator_id(actor)
        r = client.get(f"/api/embodied/segments?episode_id=demo_episode&annotator_id={ann}")
        assert r.status_code == 403

    def test_get_with_forged_signature_is_rejected(self, client):
        # #2 mirror: a forged principal signature => 403, nothing returned.
        actor = "alice"
        ann = _annotator_id(actor)
        r = client.get(
            f"/api/embodied/segments?episode_id=demo_episode&annotator_id={ann}",
